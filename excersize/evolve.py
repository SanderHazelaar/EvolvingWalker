...
class CTRNN_agent(object):
    ...
    def __init__(self, network_size, genome = [], weights=[], taus = [], gains = [], biases = []):
        ...
        if(self.network_size < self.n_observations + self.n_actions): # making sure the network is large enough for the problem
            self.network_size = self.n_observations + self.n_actions;
        self.cns = CTRNN(self.network_size)            # Creating the recurrent neural network with default values
        ...
        if(len(genome) == self.network_size*self.network_size+3*self.network_size):
            ...
            w = weight_range * (2.0 * (genome[:ind] - 0.5)) # retrieving weights from genome
            weights = np.reshape(w, [self.network_size, self.network_size]) # reshaping the weights to n x n matrix
            biases = weight_range * (2.0 * (genome[ind:ind+self.network_size] - 0.5)) # retrieving biases from genome
            ind += self.network_size
            taus = 0.9 * genome[ind:ind+self.network_size] + 0.05 # retrieving taus from genome
            ind += self.network_size
            gains = 2.0 * (genome[ind:ind+self.network_size]-0.5) # retrieving gains from genome
        
        if(len(weights) > 0):
            # weights must be a matrix size: network_size x network_size
            # Applying the network parameters to the network created by the CTRNN module
            self.cns.weights = csr_matrix(weights)
        if(len(biases) > 0):
            self.cns.biases = biases
        if(len(taus) > 0):
            self.cns.taus = taus # time constants
        if(len(gains) > 0):
            self.gains = gains   # activation function gains
